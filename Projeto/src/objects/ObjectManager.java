...
public class ObjectManager {
	...
	public void checkInsideWater(Player p) {
		// Coordenadas da área da água
		int waterX = 100; // Exemplo: coordenada x da água na tela
		int waterY = 300; // Exemplo: coordenada y da água na tela
		int waterWidth = 200; // Exemplo: largura da área da água
		int waterHeight = 50; // Exemplo: altura da área da água

		// Verifica se o jogador está dentro da área da água
		if (p.getHitbox().intersects(waterX, waterY, waterWidth, waterHeight)) {
		}
	}
	...
	public void checkObjectHit(Rectangle2D.Float attackbox) {
		for (GameContainers gc : containers)
			if (gc.isActive() && !gc.FaçaAnimation) {
	...
	private void updateCannons(int[][] lvlData, Player player) {
		for (Cannon c : cannons) {
			...
			c.update();
			if (c.getAniIndex() == 4 && c.getAnimationTimer() == 0)
				shootCannon(c);
		}
	}
